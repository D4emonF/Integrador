...
public class VoiceSelfMute extends ListenerAdapter {
    ...
    public void onGuildVoiceSelfMute(GuildVoiceSelfMuteEvent event) {
        ...
        try {
            ...
            if (event.getGuild().equals(bot.getGuildById(config.get("guild").asLong()))) {
                ...
                if (event.getVoiceState().isSelfMuted() && event.getVoiceState().inAudioChannel()){
                    paraContagemTempo(event.getMember(), dateTime);
                }
                else {
                    salvaTempoI(event.getMember(), gerarTimestamp(dateTime));
                }
            }
        } catch (IOException e) {
